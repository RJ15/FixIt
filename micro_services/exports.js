...
exports.handler = (event, context, callback) => {
    ...
    connection.connect(function(err) {
        if (err) {
            ...
            var sqlQuery = "SELECT sea.id as seasonId, sea.name as seasonName, feed.id as auctionFeedId, team.id as teamId, team.name as teamName, bid.description as bidType, player.id as playerId, player.name as playerName, feed.bid_value as price, timestamp as timestamp FROM auction_feed AS feed JOIN seasons AS sea ON feed.season_id = sea.id JOIN  teams AS team ON feed.team_id = team.id JOIN players AS player ON feed.player_id = player.id JOIN bid_type AS bid ON feed.bid_type = bid.id where feed.bid_type ='BID'"
